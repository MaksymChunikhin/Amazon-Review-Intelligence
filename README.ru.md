...
> **TL;DR.** End-to-end NLP-проект: 500k отзывов Amazon Home & Kitchen → классификация тональности
> (лучшая модель: дообученный **DistilBERT, macro-F1 0.744**, против настроенного TF-IDF baseline 0.717),
> 12 LDA-тем с аналитикой pain points, поиск расхождений «рейтинг vs текст» и двуязычный
> Streamlit-дашборд с live-предсказанием. Валидация строгая: один стратифицированный
> train/test split для всех пяти моделей, тюнинг только по CV на train, ограничения задокументированы.
> Ноутбуки в [notebooks/](notebooks/) двуязычные (EN/RU).
